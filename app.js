...
//Comprobar si hay articulos agregadps guardados en el Local Storage

let prodStorage = JSON.parse(localStorage.getItem('prodStorage'));

if(prodStorage !== null && prodStorage !== 0){
    for(const produ of prodStorage){
...
const validarDatos = () => {
    ...
    if (iNombre == '' || iPrecio == '' || iDetalle == '') {
        swal({
            text: "Error debe completar todos los campos para continuar",
            icon: "error",
            className: "error"});
...
const agregarDatos = (e) => {
    ...
    if (bandera === true) {
        ...
        localStorage.setItem(`prodStorage`, JSON.stringify(productos));
